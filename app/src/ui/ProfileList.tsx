...
export function ProfilesList() {
    ...
    return (<div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      {profiles.map((p) => {
            ...
            <button onClick={() => toggleOverlayTarget(p.id)} title={active ? "Overlay target (click to disable)" : "Mark as overlay target"} style={{
                    width: 34,
                    height: 34,
                    borderRadius: 10,
            ...
            <button onClick={() => toggleOverlaySupportTarget(p.id)} title={supportActive ? "Support target (click to disable)" : "Mark as support target"} style={{
                    width: 34,
                    height: 34,
                    borderRadius: 10,
                    border: "1px solid rgba(255,255,255,0.15)",
