...
export function ProfilesList() {
  ...
  const overlayTargetId = useMemo(
    () => profiles.find((p) => p.overlayTarget)?.id ?? null,
    [profiles]
  );
  ...
  async function toggleOverlayTarget(profileId: string) {
    ...
    // Klick auf aktives Target => deaktivieren (kein Target)
    const payload = isAlreadyTarget
      ? { profileId: null }
      : { profileId, iconKey: "aibatt-gold" };

    // Server gibt dir die aktualisierte Liste zurück (so haben wir’s im IPC gebaut)
      ...
      {profiles.map((p) => {
        ...
        return (
          ...
          >
            ...
            {/* Overlay Target Icon */}
            <button
              onClick={() => toggleOverlayTarget(p.id)}
              title={active ? "Overlay-Ziel (klicken zum deaktivieren)" : "Als Overlay-Ziel markieren"}
              style={{
                width: 34,
                height: 34,
                borderRadius: 10,
