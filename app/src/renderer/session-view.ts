...
export async function renderSession(root: HTMLElement) {
    ...
    const tr = (key: TranslationKey, replacements?: Record<string, string>) => {
        let text = t(key);
        if (replacements) {
    ...
    const showFcoinConverter = () => {
        closeToolsMenu();
        const win = window.open("", "fcoinConverter", "width=420,height=360,menubar=no,toolbar=no,resizable=yes");
        if (!win) {
            alert(tr("popup.blocked" as TranslationKey));
            return;
        }

        const intlLocale = localeToIntl(currentLocale);
...
<head>
  ...
  <style>
    ...
    body { margin: 0; font-family: system-ui, -apple-system, sans-serif; background:#0b1220; color:#e6eefc; display:flex; min-height:100vh; }
    .card { margin: auto; width: 100%; max-width: 360px; background:#0f1a33; border:1px solid #1b2b4d; border-radius: 14px; padding:18px; box-shadow:0 18px 48px rgba(0,0,0,0.45); }
    h1 { margin:0 0 12px; font-size:18px; }
    label { display:block; margin:10px 0 6px; font-weight:600; }
    input { width:100%; padding:10px 12px; border-radius:10px; border:1px solid #294093; background:#0d1830; color:#e6eefc; font-size:14px; }
    input:focus { outline:2px solid #3b7bff; border-color:#3b7bff; }
    .row { display:flex; gap:10px; }
    .muted { color:#8aa0d0; margin-top:8px; font-size:13px; }
  </style>
</head>
<body>
  <div class="card">
    ...
    <div class="muted" id="hint">${fcoinTexts.hint}</div>
  </div>
  <script>
    ...
    const showShoppingList = () => {
        ...
        const intlLocale = localeToIntl(currentLocale);
        const apiLocale = currentLocale === "cn" ? "cn" : currentLocale === "jp" ? "jp" : currentLocale;
        const fmt = new Intl.NumberFormat(intlLocale);
        const shoppingTexts = {
...
<style>
:root{color-scheme:dark;--ar:44,107,255;--bg:#0b1220;--text:#e6eefc}
...
searchInput.focus();
    ...
    const toolEntries: ToolEntry[] = [
        {
            label: tr("tools.menu.fcoin" as TranslationKey),
    ...
    tabsBar.append(tabsSpacer, tabsProgress, splitControls, btnTabHeight, btnTools, btnEditMode, btnSaveLayout, btnLayouts, btnSplit);
