...
export type OverlayHudLayout = {
  offsetX: number; // relativ zur Game-Rect (screen coords)
  offsetY: number;
  width: number;
  height: number;
};
...
async function readProfiles(): Promise<Profile[]> {
  try {
    ...
    // ✅ Migration: settings + hud defaults sicherstellen
...
export function createProfilesStore() {
  return {
    ...
    async setOverlayTarget(profileId: string | null, iconKey?: string): Promise<Profile[]> {
      ...
      const next = ps.map((p) => {
        if (!profileId) return { ...p, overlayTarget: false };
        if (p.id !== profileId) return { ...p, overlayTarget: false };
        return {
          ...p,
          overlayTarget: true,
