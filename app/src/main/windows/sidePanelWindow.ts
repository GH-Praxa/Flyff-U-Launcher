...
export function createSidePanelWindow(parent: BrowserWindow, opts?: {
    ...
    const win = new BrowserWindow({
        ...
        webPreferences: {
            ...
            // NodeIntegration aus für bessere Sicherheit - preload Bridge wird verwendet.
...
<script>
  ...
  const ROI_ITEMS = [
    { key: "lvl", label: "Level", target: "fighter" },
    { key: "charname", label: "Charakter", target: "fighter" },
    { key: "exp", label: "EXP-Prozent", target: "fighter" },
    { key: "lauftext", label: "Lauftext", target: "fighter" },
    { key: "rmExp", label: "RM EXP", target: "support" },
    { key: "enemyName", label: "Gegner Level/Element", target: "fighter" },
    { key: "enemyHp", label: "Gegner HP", target: "fighter" },
  ];
  ...
  async function openRoiCalibratorFor(key){
    try{
      ...
      if(!pid) {
        ...
        alert(key === "rmExp"
          ? "Kein Support-Ziel gesetzt. Bitte zuerst ein Profil als Support-Ziel wählen."
          : "Kein Overlay-Ziel gesetzt. Bitte zuerst ein Profil als Overlay-Ziel wählen.");
        return;
      }
      const ok = await invokeMain("roi:open", key ? { profileId: pid, roiKey: key } : pid);
      if (!ok) {
        alert("ROI-Kalibrierung konnte nicht gestartet werden.");
      }
      renderRoiPanel();
    }catch(e){
      console.error("roi:open failed", e);
      alert("ROI-Kalibrierung fehlgeschlagen: " + (e?.message || e));
    }
  }
  ...
  function renderCoreTab(name) {
    ...
    const buildOcrSection = () => {
      ...
      heading.textContent = "OCR Live-Werte";

      const ocrItems = [
        { key: "lvl", label: "Level", format: v => (v === null || v === undefined || v === "" ? "-" : String(v)), unit: "ms" },
        { key: "exp", label: "EXP", format: v => (v === null || v === undefined || v === "" ? "-" : String(v)), unit: "ms" },
        { key: "charname", label: "Charakter", format: v => (v === null || v === undefined || v === "" ? "-" : String(v)), unit: "ms" },
        { key: "lauftext", label: "Lauftext", format: v => (v === null || v === undefined || v === "" ? "-" : String(v)), unit: "ms" },
        { key: "rmExp", label: "RM EXP", format: v => (v === null || v === undefined || v === "" ? "-" : String(v)), unit: "ms", target: "support" },
        { key: "enemyName", label: "Gegner Level/Element", format: v => (v === null || v === undefined || v === "" ? "-" : String(v)), unit: "ms" },
        { key: "enemyHp", label: "Gegner HP", format: v => (v === null || v === undefined || v === "" ? "-" : String(v)), unit: "ms" },
      ];
      ...
      ocrItems.forEach(item => {
        ...
        if (item.key === "lvl") {
          ...
          manualLabel.textContent = "Killfeed Level:";
          ...
          manualLevelInput.placeholder = "1-300";
          manualLevelInput.value = "";
          manualLevelInput.disabled = true;
          ...
          manualToggleText.textContent = "Manuell";
        ...
        if (item.key === "exp") {
          ...
          manualExpButton.textContent = "Setzen";
          manualExpButton.disabled = true;
      ...
      note.className = "hint";
      note.textContent = "Timer = 0 deaktiviert den Scan. Werte in Millisekunden.";
      ...
      const refreshOcr = async () => {
        try {
          ...
          if (!profileId && !supportProfileId) {
            note.textContent = "Kein Overlay-Ziel gesetzt.";
            ocrItems.forEach((item) => {
          ...
          const noteParts = [
            profileId ? (isStaleFighter ? "Profil: " + profileId + " (veraltet)" : "Profil: " + profileId) : null,
            supportProfileId ? (isStaleSupport ? "Support: " + supportProfileId + " (veraltet)" : "Support: " + supportProfileId) : null,
          ].filter(Boolean);
          note.textContent = noteParts.join(" | ") || "Kein Overlay-Ziel gesetzt.";
        } catch (err) {
          note.textContent = "OCR nicht verfuegbar";
        }
      };
    ...
    const buildRoiSection = () => {
      ...
      const heading = document.createElement("div");
      heading.className = "sectionTitle";
      heading.textContent = "OCR-Regionen (ROI)";
      ...
      allToggleLabel.textContent = "Alle anzeigen";
      ...
      const rows = ROI_ITEMS.map(item => {
        ...
        text.textContent = "Nicht gesetzt";
        ...
        btnDebug.onclick = async () => {
          ...
          if (!pid) {
            alert("Kein Profil für dieses ROI gesetzt.");
            return;
          }
          btnDebug.disabled = true;
          btnDebug.textContent = "Speichere...";
          try{
            const filePath = await invokeMain("roi:debug:save", { profileId: pid, key: item.key });
            alert("Debug-Screenshot gespeichert unter:\\n" + (filePath || ""));
          }catch(err){
            console.error("roi:debug:save failed", err);
            alert("Debug-Screenshot fehlgeschlagen: " + (err?.message || err));
          }finally{
        ...
        toggleLabel.textContent = "ROI anzeigen";
      ...
      manualTitle.textContent = "ROI-Werte direkt bearbeiten";
      ...
      manualSelectLabel.textContent = "Feld";
      ...
      ([
        { key: "x", label: "X (0..1)" },
        { key: "y", label: "Y (0..1)" },
        { key: "w", label: "Breite (0..1)" },
        { key: "h", label: "Höhe (0..1)" },
      ]).forEach(field => {
      ...
      manualDeleteBtn.textContent = "Entfernen";
      ...
      manualFeedback.textContent = "Regler bewegen (0..1) für Live-ROI.";
      ...
      async function applyManualValues({ silent = false } = {}) {
        ...
        if (!pid) {
          if (!silent) notifyManual("Bitte ein Overlay-Profil wählen.", "error");
          return false;
        }
        const normalized = gatherManualValues();
        if (!normalized) {
          notifyManual("Ungültige Koordinaten.", "error");
          return false;
        }
        ...
        try {
          ...
          const message = silent ? "Live-ROI aktualisiert." : label + " gespeichert.";
          ...
          notifyManual("Live-ROI konnte nicht gespeichert: " + (reason || "unbekannter Fehler"), "error");
          return false;
        }
      }
      ...
      manualSelect.addEventListener("change", () => {
        ...
        updateManualInputs();
        notifyManual("Werte können angepasst werden.", "info");
      });
      ...
      manualDeleteBtn.addEventListener("click", async () => {
        const pid = profileIdForKey(manualActiveKey);
        if (!pid) {
          notifyManual("Bitte ein Overlay-Profil wählen.", "error");
          return;
        }
        if (!roisForKey(manualActiveKey)?.[manualActiveKey]) {
          notifyManual("Kein ROI zum Entfernen vorhanden.", "error");
          return;
        }
        ...
        try {
          ...
          notifyManual(label + " entfernt.", "success");
        } catch (err) {
          console.error("[ROI] manual delete failed", err);
          notifyManual("Löschen fehlgeschlagen.", "error");
        } finally {
      ...
      const note = document.createElement("div");
      note.className = "hint";
      note.textContent = "Status wird geladen...";
      ...
      const applyStatus = (roisFighter, roisSupport, visFighter, visSupport, fighterId, supportId) => {
        ...
        rows.forEach(r => {
          ...
          r.text.textContent = pid ? "" : "Kein Profil";
          if (r.checkbox) {
      ...
      async function refresh() {
        try{
          ...
          const noteText = [
            fighterId ? "Overlay-Profil: " + fighterId : null,
            supportId ? "Support: " + supportId : null,
          ].filter(Boolean).join(" | ");
          note.textContent = noteText || "Kein Overlay-Ziel gesetzt.";
          applyStatus(fighterRois || {}, supportRois || {}, fighterVis || {}, supportVis || {}, fighterId, supportId);
        }catch(_err){
          note.textContent = "ROI-Status konnte nicht geladen werden.";
        }
      }
  ...
  // Extend renderTab for plugin tabs
  async function renderTab(name) {
    if (name && name.startsWith("plugin:")) {
      ...
      if (!pluginTab) {
        content.innerHTML = "<div class='section'><div class='hint'>Plugin tab not found</div></div>";
        return;
      }
      ...
      // Create iframe for plugin content
      content.innerHTML = "";
      ...
      iframe.addEventListener("load", () => {
        applyThemeToPluginDocument(iframe.contentDocument || iframe.contentWindow?.document);
      });

      // Inline plugin HTML/CSS/JS to avoid file:// loading being blocked
      try {
        ...
        const stripJs = new RegExp("<script[^>]*ui_sidepanel\\.js[^>]*>" + scriptCloseTag, "i");
        html = html.replace(stripCss, "");
        html = html.replace(stripJs, "");
        if (pluginTab.baseHref && html.includes("<head>") && !/<base\\s/i.test(html)) {
          // Use single quotes to avoid breaking the script string
        ...
        if (pluginTab.js) {
          html = html.replace("</body>", "<script>" + pluginTab.js + scriptCloseTag + "</body>");
        }
        const bridgeScript =
          "<script>" +
          "window.__overlayTargetId=" + (overlayTargetId ? JSON.stringify(overlayTargetId) : "null") + ";" +
        ...
        console.error("[HudPanel] Failed to inline plugin tab assets", err);
        iframe.srcdoc = pluginTab.html;
      }
