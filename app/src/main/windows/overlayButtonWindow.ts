...
export function createOverlayButtonWindow(opts: {
  ...
  const handleToggle = (payload: any) => {
    ...
    // 2) Wenn du zusätzlich/alternativ ein zentrales Toggle-System hast,
...
<script>
  ...
  // Aktuelles Profil (vom Main-Prozess gesetzt)
  window.__profileId = ${JSON.stringify(opts.profileId ?? "")};

  // Ermöglicht späteres Aktualisieren der Profil-ID durch den Main-Prozess
  ...
  // Klick sendet Toggle an Main; bei fehlendem preload via Console-IPC
  function sendToggle(){
