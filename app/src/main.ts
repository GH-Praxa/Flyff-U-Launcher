...
app.whenReady().then(async () => {
    ...
    const syncRoiOverlay = async () => {
        try {
            ...
            if (!roiOverlayWindow || roiOverlayWindow.isDestroyed()) {
                roiOverlayWindow = createOverlayWindow(host.parent, { preloadPath });
    ...
    const syncRoiSupportOverlay = async () => {
        try {
            ...
            if (!roiSupportOverlayWindow || roiSupportOverlayWindow.isDestroyed()) {
                roiSupportOverlayWindow = createOverlayWindow(host.parent, { preloadPath, role: "support" });
    ...
    const maybeAutoIncrementManualLevel = async (profileId: string): Promise<void> => {
        const manual = getManualLevelOverride(profileId);
        if (!manual) return;
        const nextValue = clampManualLevel(manual.value + 1);
        if (nextValue === manual.value) return;
        const updated = await saveManualLevelOverride(profileId, { value: nextValue });
        if (!updated) return;
    ...
    const handleExpLevelUp = (
        ...
        if (levelUpByThreshold || levelUpByDrop) {
            ...
            void maybeAutoIncrementManualLevel(profileId).catch((err) => logErr(err, "OCR auto level up"));
        }
    };
