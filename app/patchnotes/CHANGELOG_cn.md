...
与 Android 应用（v33）同步，PC 启动器获得相同的控制器改进：

**新的 D-pad 默认设置**
- D-pad ↑/↓ → **放大 / 缩小**（屏幕中央的合成鼠标滚轮）
- D-pad ←/→ → **标签切换**（`@prevTab`/`@nextTab`）
- D-pad-上之前硬编码到 action-pad — 现在 `@actionPad` 必须显式映射到某个按钮（例如 Select）

**鼠标模式（光标）现在可配置**
- L2 之前硬编码为「为光标模式保留」；现在它是一个特殊操作 `@cursorHold`（L2 的默认设置）
- 按住 `@cursorHold` 时：右摇杆移动合成鼠标光标，A 按钮 = 在当前位置左键单击
- 替代方案 `@cursorToggle`：点击在普通模式和光标模式之间切换
- 两者都可以映射到任何按钮 — 如果你想将 L2 用作技能槽，请将光标模式映射到例如 R2 或 L1
- L2 现在也可以作为修饰符层槽使用（之前被排除）
